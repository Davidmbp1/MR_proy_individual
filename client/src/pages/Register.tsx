...
function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessBanner('Registered successfully! Redirecting...');
      setLoading(false);
      ...
      setLoading(false);
      setMessage(err.response?.data?.message || 'Error in register');
    }
  };
  ...
  const handleGoogleSuccess = async (response: any) => {
    try {
      ...
      setSuccessBanner('Google register successful! Redirecting...');
      setTimeout(() => {
        navigate('/complete-profile', { replace: true });
      }, 1500);
    } catch (error: any) {
      setMessage('Error in Google register');
    }
  };
    ...
    <div className="relative min-h-screen flex">
      <div className="w-full md:w-1/2 bg-white flex flex-col justify-center px-6 py-8">
        ...
        <div className="max-w-lg mx-auto w-full">
          <h2 className="text-3xl font-bold text-blue-900 mb-3">Sign Up</h2>
          <p className="text-gray-700 mb-4">Create an account for LastMinuteFoods!</p>
          ...
          <p className="mt-6 text-gray-600">Or register with Google:</p>
